...
After the PHP compile was done, I set out to continue with ``php-gtk``. For this I had to install a few
more packages: libglade-dev, libglib-dev and m4. Of both compiles I made
packages that you can just install on the OpenMoko: ``php`` and ``php-gtk``.
In order to demonstrate that it actually works, I've also a demo
application "CountDown" available as ``package``.
