The Other Side of the Firehose
==============================

.. articleMetaData::
   :Where: London, UK
   :Date: 2026-06-09 14:45 Europe/London
...
humans with LLM assisted tools, focussing on security related problems, from
...
But the issue is not a real-live problem, insofar this can only happen if you
...
through ``-r``, the CLI binary does not generate ``EXT_STMT`` opcodes, which
would otherwise prevent this situation from happening.
...
<https://bugs.xdebug.org>`_, which is a similar report, but then for
...
problem. It relied on the AddressSanitizer to show any problems, but I could
...
Luckily, using the Xdebug test suite with the ``valgrind`` tool clearly showed
the problem.
